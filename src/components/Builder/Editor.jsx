...
export function Editor() {
    ...
    return (
        <section className={`fixed top-0 right-0 min-h-screen w-[400px] dark:bg-slate-700 bg-white z-10 transition-all duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
            <button className={`absolute right-full dark:bg-slate-700 bg-white top-8 p-1 font-xl transition-all duration-300 ${open ? 'rotate-180' : 'rotate-0'}`} onClick={() => setOpen(!open)}>
